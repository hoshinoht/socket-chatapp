...
class DebugLock:
  ...
  def acquire(self, blocking=True, timeout=-1):
    ...
    if DEBUG and result:
      ...
      debug_print(
          f"Attempting to acquire lock at {caller.filename}:{caller.lineno}")
  ...
  try:
    ...
    if username in clients:
      ...
      try:
        ...
        debug_print(
            f"sendall failed, using regular send: {sendall_error}", username)
...
def broadcast(message, exclude_conn=None):
  ...
  with lock:
    debug_print(
        f"In broadcast critical section, clients count: {len(clients)}")
    # Make a copy to avoid dict changes during iteration
...
def clientthread(conn, addr):
  ...
  try:
    ...
    with lock:
      ...
      if username in clients:
        debug_print(
            f"User {username} is already logged in, rejecting duplicate connection", username)
      ...
      try:
        ...
        if ready[0]:
          ...
          if not encrypted_message:
            debug_print(
                f"Empty message from {username}, closing connection.", username)
            break
          ...
          try:
            ...
            if message_str.startswith('@'):
              ...
              debug_print(
                  f"Broadcasting regular message from {username}", username)
              broadcast(username + ": " + message_str, conn)
          except Exception as e:
            debug_print(
                f"Error processing message from {username}: {e}", username)
            traceback.print_exc()
            send_to_client(
                username, f"Error processing your message: {str(e)}")
    ...
    try:
      conn.close()
      debug_print(
          f"Closed socket for {username or 'unknown'}", username or "unknown")
    except:
      debug_print(
          f"Error closing socket for {username or 'unknown'}", username or "unknown")
    debug_print(
        f"Connection closed for {username or 'unknown'} from {addr}", username or "unknown")
...
def handle_history(username, args, conn):
  ...
  try:
    ...
    if not user_hist:
      ...
      filtered_hist = [
          msg for msg in user_hist if not msg.startswith("--- Last")]

      # Create the header separately
      hist_header = "\n--- Last {} Messages ---\n".format(
          min(N, len(filtered_hist)))
...
def handle_group_set(username, args, conn):
  ...
  if len(args) < 2:
    send_to_client(
        username, "Usage: @group set <group_name> <user1>,<user2>,...\n")
    return
  ...
  if group_exists:
    send_to_client(username, "ERROR: Group " +
                   group_name + " already exists.\n")
  elif not_online:
    send_to_client(username, "ERROR: These users not online: " +
                   ", ".join(not_online) + "\n")
  else:
...
def handle_group_send(username, args, conn):
  ...
  if not group_exists:
    ...
    send_to_client(
        username, "ERROR: You are not a member of group " + group_name + ".\n")
  else:
    full_msg = "[Group: " + group_name + "] " + \
        username + ": " + group_msg + "\n"
...
def process_command(username, message_str, conn):
  ...
  if message_str.startswith('@group'):
    ...
    # Split into max 4 parts: @group, subcmd, group_name, [rest]
...
def close_all_connections():
  ...
  # Also clear the clients dictionary
  with lock:
    clients.clear()  # Replace `online_users` with `clients`
    debug_print("Cleared clients list")
